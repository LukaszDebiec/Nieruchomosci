...
export class HttpPropertiesBackendService extends PropertiesBackendService {
    ...
    private getAllPropertiesUrl: string = "api/property/getproperties";
    ...
    addProperty(newProperty: Property): Observable<number> {
        return this.http.post(this.addPropertyUrl, JSON.stringify(newProperty), this.jsonContentOptions).map(response => response.json() as number);
    }
    getProperty(id: number): Observable<Property> {
        return this.http.get(this.getPropertyUrl + id, this.jsonContentOptions).map(response => response.json());
    }
    getProperties(): Observable<Property[]> {
        return this.http.get(this.getAllPropertiesUrl, this.jsonContentOptions).map(response => response.json());
    }
    ...
    deleteProperty(propertyId: number): Observable<number> {
        return this.http.get(this.deletePropertyUrl + propertyId, this.jsonContentOptions).map(response => response.json());
    }
